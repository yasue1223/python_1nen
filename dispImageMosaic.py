...
def dispPhoto(path): #指定されたパスの画像を読み込んでリサイズし、画像ラベルに表示する処理を行います。
    newImage = PIL.Image.open(path).convert('L').resize((32,32)).resize((300,300),resample=0) #指定されたパスの画像をPIL.Image.open()メソッドで読み込みモノクロに変換、resize()メソッドでリサイズを繰り返してモザイク画像にする。
    #resampleパラメータには0が指定されています。これにより、リサンプリング時にニアレストネイバー法が使用されます。
